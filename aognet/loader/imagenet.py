...
def imagenet_iterator(cfg, kv):
    ...
    train = mx.io.ImageRecordIter(
            ...
            min_random_scale    = 1.0 if cfg.dataset.data_type in ["cifar10", "cifar100"] else 1.0 if cfg.dataset.aug_level == 1 else 0.533,  # 256.0/480.0
            max_aspect_ratio    = 0 if cfg.dataset.data_type in ["cifar10", "cifar100"] else 0 if cfg.dataset.aug_level == 1 else 0.25,
            random_h            = 0 if cfg.dataset.data_type in ["cifar10", "cifar100"] else 0 if cfg.dataset.aug_level == 1 else 36,  # 0.4*90
            random_s            = 0 if cfg.dataset.data_type in ["cifar10", "cifar100"] else 0 if cfg.dataset.aug_level == 1 else 50,  # 0.4*127
            random_l            = 0 if cfg.dataset.data_type in ["cifar10", "cifar100"] else 0 if cfg.dataset.aug_level == 1 else 50,  # 0.4*127
            ...
            shuffle             = True if cfg.dataset.aug_level > -1 else False,
